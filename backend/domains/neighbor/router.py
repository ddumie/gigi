...
def create_group_search_final(post: GroupSearchCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def delete_group_search_final(post_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_my_group_search_final(db: Session = Depends(get_db),  current_user: User = Depends(get_current_user)): 
    return get_my_group_search_logic(user_id=current_user.id, db=db)

#my posts 페이지에서 내가 쓴 습관도 보여주기
@router.get("/feed/my", response_model=list[FeedPostResponse])
def get_my_habits_final(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)): 
    return get_my_habits_logic(user_id=current_user.id, db=db)
...
@router.post("/feed")
def create_habit_feed_final(body: HabitFeedCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return create_habit_feed_logic(habit_id=body.habit_id, content=body.content, user_id=current_user.id, db=db)
...
def delete_habit_feed_final(post_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return delete_habit_feed_logic(post_id=post_id, user_id=current_user.id, db=db)


# 피드 단건 조회
@router.get("/feed/{post_id}", response_model=FeedDetailResponse)
def get_feed_detail_final(post_id: int, db: Session = Depends(get_db)):
...
def get_feed_comments_final(post_id: int, db: Session = Depends(get_db)):
    return get_feed_comments_logic(post_id=post_id, db=db)

# 댓글 작성
@router.post("/feed/{post_id}/comments")
def create_feed_comment_final(
    post_id: int,
